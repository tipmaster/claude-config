...
description: Use this agent when you need to create, refine, or optimize prompts for any LLM (OpenAI, Anthropic, Gemini, etc.). This includes writing new prompts from scratch, improving existing prompts for better outputs, adapting prompts across different models, or troubleshooting prompt performance issues. Examples:\n\n<example>\nContext: User needs help creating a prompt for content generation.\nuser: "I need a prompt that will generate product descriptions for an e-commerce site"\nassistant: "I'll use the prompt-engineer agent to craft an effective prompt for product descriptions."\n<commentary>\nSince the user needs a specialized prompt created, use the Task tool to launch the prompt-engineer agent.\n</commentary>\n</example>\n\n<example>\nContext: User has a prompt that isn't working well.\nuser: "My ChatGPT prompt keeps giving me overly verbose responses, can you fix it?"\nassistant: "Let me use the prompt-engineer agent to optimize your prompt for more concise outputs."\n<commentary>\nThe user needs prompt optimization, so launch the prompt-engineer agent to refine their existing prompt.\n</commentary>\n</example>\n\n<example>\nContext: User wants to adapt a prompt for different models.\nuser: "I have this Claude prompt but need it to work well with Gemini too"\nassistant: "I'll engage the prompt-engineer agent to adapt your prompt for cross-model compatibility."\n<commentary>\nCross-model prompt adaptation requires specialized knowledge, use the prompt-engineer agent.\n</commentary>\n</example>
model: sonnet
color: blue
---
